...
std::vector<UsiMoveCandidate> generate_candidates(size_t move_index, bool is_gote) {
    ...
    // 🟢 本物の最善手だけを唯一の候補手（target_state_idx = move_index）として追加！
    // ダミー候補手を生成しないため、ポテンシャルの比較誤爆による反則手（2g2fなど）は100%原理的に発生しなくなります。
...
int main() {
    ...
    int current_sfen_start_move = 1; // SFEN開始手数（デフォルトは1手目）
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (cmd == "usi") {
            ...
            if (has_sfen) {
                ...
                // sfen [盤面] [手番: b/w] [持ち駒] [開始手数] のパース
                if (sfen_idx != 99999) {
                    if (sfen_idx + 2 < tokens.size()) {
                        if (tokens[sfen_idx + 2] == "w") {
                            sfen_is_gote = true;
                        } else {
                            sfen_is_gote = false;
                        }
                    }
                    if (sfen_idx + 4 < tokens.size()) {
                        try {
                            current_sfen_start_move = std::stoi(tokens[sfen_idx + 4]);
                        } catch (...) {
                            current_sfen_start_move = 1;
                        }
                    }
                    ...
                    // moves以降のパース
                    bool in_moves = false;
                    for (size_t i = sfen_idx + 5; i < tokens.size(); ++i) {
                        if (tokens[i] == "moves") {
                            in_moves = true;
                            continue;
                        }
            ...
            // 絶対的な手順数（move_index）の同期計算
            size_t move_index = (current_sfen_start_move - 1) + current_moves.size();
